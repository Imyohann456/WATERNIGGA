...
local Sploit = Instance.new("ScreenGui")
...
local Animations = {
    ...
    CoolAnim = {
        {CFrame = CFrame.new(0, 0, 0) * CFrame.Angles(math.rad(360 * time), math.rad(0), math.rad(100)), Time = 0.1},
        {CFrame = CFrame.new(0, 0, 0) * CFrame.Angles(math.rad(360 * time), math.rad(0), math.rad(0)), Time = 0.1}
	}
}
...
task.spawn(function()
	...
	task.spawn(function()
		...
		DropDownButton.Activated:Connect(function()
			...
			if Value == true then
				InstanceUI = CreateSlider(CombatTab, "Range", Settings.KillAura.Range, 30, LayoutOrder + 4, function(CallBack)
...
task.spawn(function()
	local Reach, DropDownButton, LayoutOrder, UIGradient = CreateToggle(CombatTab, "Reach", Settings.Reach.Value, function(CallBack)
		...
		if Settings.Reach.Value == true then
			CombatConstant.RAYCAST_SWORD_CHARACTER_DISTANCE = 30
		end
